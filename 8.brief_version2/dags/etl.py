...
def transform_data(**kwargs):
    ti = kwargs['ti']
    data = ti.xcom_pull(task_ids='extract_task')  # récupère le JSON du task extract
    df = pd.read_json(data)

    # Exemple de transformation : nettoyage des données
    df.dropna(inplace=True)
    df['date'] = pd.to_datetime(df['dt_creation'], errors='coerce')
    df = df[df['date'].notnull()]
    
    print(f"Données transformées : {df.shape[0]} lignes après nettoyage")

    # On renvoie le DataFrame transformé en JSON pour la prochaine étape
    return df.to_json()
